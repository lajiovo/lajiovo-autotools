...
async def handle_op_command(
    ...
    # ------------------- #op log 日志查看相关指令 -------------------
        ...
        if p_action == "set":
            ...
            # 1. 直接读取 groupId 为 "push" 的历史记录列表（兼顾字典和对象属性两种结构）
            if isinstance(client.chat_mgr.group_history, dict):
                push_history = client.chat_mgr.group_history.get("push", [])
            else:
                push_history = getattr(client.chat_mgr.group_history, "push", [])
            ...
            for idx, item in enumerate(selected_items, start=start_idx):
                ...
                msg_content = item.get("content", "无内容")
                res_lines.append(f"-------------------------\n📌 [#{idx}] 🕒 {time_str}\n{msg_content}")
