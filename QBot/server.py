...
def _get_stored_key():
    ...
    if not os.path.exists(KEY_FILE_PATH):
        _log.warning(f"⚠️ 校验配置文件不存在: {KEY_FILE_PATH}，接口鉴权功能将不可用！")
        return None
    try:
        with open(KEY_FILE_PATH, "r", encoding="utf-8") as f:
            ...
            if not raw_key:
                _log.warning("⚠️ key.json 中未配置 key 字段或内容为空！")
                return None
            return raw_key
    except Exception as e:
        _log.error(f"❌ 读取 key.json 失败: {e}")
        return None
...
def start_http_servers(client_instance):
    ...
    class ControlRequestHandler(BaseHTTPRequestHandler):
        ...
        def _serve_static_file(self, relative_path):
            ...
            try:
                ...
                _log.error(f"读取文件失败: {file_path}, 错误: {e}")
        ...
        def do_GET(self):
            ...
            if path_part.startswith("/assets/"):
                return self._serve_static_file(path_part.lstrip("/"))
            ...
            # 获取用户列表
            if path_part == "/api/users":
                users = client_instance.get_user_list()
                self._send_json({"status": "success", "data": users})
                return

            # 获取群聊列表
            if path_part == "/api/groups":
                groups = client_instance.get_group_list()
                self._send_json({"status": "success", "data": groups})
                return

            # 获取聊天历史记录
            if path_part == "/api/history":
                ...
                history = client_instance.get_chat_history(target_id, is_group=is_group)
                self._send_json({"status": "success", "data": history})
                return

            # 获取昵称
            if path_part == "/api/nickname":
                target_id = query_params.get("target_id", [""])[0]
                is_group = query_params.get("is_group", ["false"])[0].lower() == "true"
                nickname = client_instance.get_nickname(target_id, is_group=is_group)
                self._send_json({"status": "success", "nickname": nickname})
                return

            # 轮询获取是否有新消息
            if path_part == "/api/check_new":
                reset = query_params.get("reset", ["true"])[0].lower() == "true"
                status_info = client_instance.check_has_new_message(reset=reset)
    ...
    _log.info("🌐 控制、API 与 Web 托管服务已启动 (端口 25567)")
